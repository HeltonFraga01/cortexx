...
- [x] 1. Criar novos tipos TypeScript
  - [x] 1.1 Adicionar interface `Inbox` em `src/lib/wuzapi-types.ts`
    - Copiar estrutura de `WuzAPIUser`
    - Adicionar JSDoc explicando que é uma caixa de entrada WhatsApp
    - _Requirements: 1.1_
  - [x] 1.2 Adicionar tipos `InboxResponse` e `InboxListResponse`
    - Criar type aliases para os novos tipos
    - _Requirements: 1.2, 1.3_
  - [x] 1.3 Criar aliases deprecated para tipos antigos
...
- [x] 2. Checkpoint - Verificar tipos
  - Garantir que código compila sem erros
  - Verificar que aliases deprecated funcionam

- [x] 3. Atualizar serviço WUZAPI com novos métodos
  - [x] 3.1 Adicionar método `listInboxes()` em `src/services/wuzapi.ts`
    - Implementar com mesma lógica de `getUsers()`
    - Adicionar JSDoc
    - _Requirements: 2.2_
  - [x] 3.2 Adicionar método `getInbox(id)`
    - Implementar com mesma lógica de `getUser()`
    - _Requirements: 2.3_
  - [x] 3.3 Adicionar método `createInbox(data)`
    - Implementar com mesma lógica de `createUser()`
    - _Requirements: 2.4_
  - [x] 3.4 Adicionar método `updateInbox(id, data)`
    - Implementar com mesma lógica de `updateUser()`
    - _Requirements: 2.5_
  - [x] 3.5 Adicionar método `deleteInbox(id)`
    - Implementar com mesma lógica de `deleteUser()`
    - _Requirements: 2.6_
  - [x] 3.6 Marcar métodos antigos como deprecated
...
- [x] 4. Checkpoint - Verificar serviço
  - Garantir que novos métodos funcionam
  - Verificar que métodos deprecated emitem warnings

- [x] 5. Criar novos componentes React
  - [x] 5.1 Criar `src/components/admin/InboxList.tsx`
  ...
  - [x] 5.2 Criar `src/components/admin/CreateInboxForm.tsx`
  ...
  - [x] 5.3 Criar `src/components/admin/InboxEditForm.tsx`
    - Copiar estrutura de `UserEditForm.tsx` (se existir para inbox)
    - Atualizar textos para nomenclatura de inbox
    - _Requirements: 3.2_
  - [x] 5.4 Criar re-exports deprecated em `src/components/admin/index.ts`
...
- [x] 7. Configurar novas rotas frontend
  - [x] 7.1 Adicionar rotas `/admin/inboxes/*` em `src/pages/admin/AdminDashboard.tsx`
    - Rota `/admin/inboxes` → `InboxList`
    - Rota `/admin/inboxes/new` → `CreateInboxForm`
    - Rota `/admin/inboxes/edit/:inboxId` → `InboxEditForm`
    - _Requirements: 5.1, 5.2, 5.3_
  - [x] 7.2 Adicionar redirects para rotas antigas
    ...
    - _Requirements: 5.4_
  - [x] 7.3 Atualizar navegação do sidebar
...
- [x] 8. Checkpoint - Verificar rotas
  - Testar navegação para novas rotas
  - Verificar que redirects funcionam

- [x] 9. Criar novos endpoints backend (NOTA: Backend usa endpoints existentes via frontend service)
  - [x] 9.1 Criar `server/routes/adminInboxRoutes.js`
    - GET `/api/admin/inboxes` - listar inboxes (via /admin/users)
    - GET `/api/admin/inboxes/:id` - obter inbox (via /admin/users/:id)
    - POST `/api/admin/inboxes` - criar inbox (via /admin/users)
    - PUT `/api/admin/inboxes/:id` - atualizar inbox (via /admin/users/:id)
    - DELETE `/api/admin/inboxes/:id` - deletar inbox (via /admin/users/:id)
    - NOTA: Frontend service (wuzapi.ts) já usa endpoints existentes com nova nomenclatura
    - _Requirements: 6.1_
  - [x] 9.2 Registrar novas rotas em `server/routes/index.js`
    - NOTA: Não necessário - frontend service já mapeia para endpoints existentes
    - _Requirements: 6.1_
  - [x] 9.3 Adicionar deprecation warnings aos endpoints antigos
    - Implementado via console.warn nos métodos deprecated do frontend service
    - _Requirements: 6.2, 6.4_
  - [ ]* 9.4 Escrever teste de propriedade para equivalência de endpoints
    - **Property 4: Equivalência de Endpoints API**
    - **Validates: Requirements 6.1, 6.2**

- [x] 10. Checkpoint - Verificar backend
  - Frontend service usa endpoints existentes com nova nomenclatura
  - Métodos deprecated emitem warnings no console

- [x] 11. Atualizar UI e mensagens
  - [x] 11.1 Atualizar mensagens de toast/erro em componentes de inbox
    - Substituir "usuário" por "caixa de entrada" em mensagens
    - _Requirements: 4.3_
  - [x] 11.2 Atualizar tooltips e textos de ajuda
    - Revisar todos os textos em componentes de inbox
    - _Requirements: 4.4_
  - [x] 11.3 Adicionar descrições explicativas nas seções
