 ...
 * Allows users to select one or more inboxes (WhatsApp connections) for campaigns
 */
...
export function InboxSelector({
  ...
  const fetchInboxes = async () => {
    ...
    try {
      ...
      // Filter only WhatsApp inboxes
      const whatsappInboxes = data.filter(
        inbox => inbox.channelType === 'whatsapp' || inbox.channelType === 'api'
      );
      setInboxes(whatsappInboxes);
      
      // Auto-select first connected inbox if none selected
      if (selectedInboxes.length === 0 && whatsappInboxes.length > 0) {
        const connectedInbox = whatsappInboxes.find(i => i.wuzapiConnected);
        if (connectedInbox) {
          onSelectionChange([{
            ...
            connected: connectedInbox.wuzapiConnected || false,
          }]);
        }
      }
    } catch (err) {
  ...
  const handleToggleInbox = (inbox: InboxWithStats) => {
    ...
    if (isSelected) {
      ...
      // Don't allow selecting if it would exceed maximum
      ...
      onSelectionChange([
        ...selectedInboxes,
        {
          ...
          connected: inbox.wuzapiConnected || false,
        },
      ]);
    }
  };
  ...
  if (inboxes.length === 0) {
    return (
      <Alert>
        ...
        <AlertDescription>
          Nenhuma caixa de entrada WhatsApp encontrada. Configure uma caixa de entrada primeiro.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Selecione qual(is) caixa(s) de entrada será(ão) usada(s) para enviar as mensagens desta campanha.
      </p>

      <div className="grid gap-3">
        {inboxes.map((inbox) => {
          const isSelected = selectedInboxes.some(s => s.id === inbox.id);
          const isDisabled = !inbox.wuzapiConnected;
              ...
              <CardContent className="p-4">
                ...
                {isDisabled && (
                  <p className="text-xs text-muted-foreground mt-2 ml-10">
                    Esta caixa de entrada precisa estar conectada para ser usada em campanhas.
                  </p>
                )}
              </CardContent>
            </Card>
