/**
 * TeamDialogUser Component
 * 
 * Create/edit team form with member management for user dashboard.
 * Reuses logic from admin TeamDialog.
 * 
...
import { Users, Loader2, UserPlus, UserMinus, Search, AlertTriangle } from 'lucide-react'
...
export function TeamDialogUser({ 
  open, 
  onOpenChange, 
  team, 
  onSuccess,
  ...
  const handleSubmit = async (data: TeamFormData) => {
    if (!isEditing && maxTeamsReached) {
      toast.error('Limite de equipes atingido. Faça upgrade do seu plano.')
      return
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        {!isEditing && maxTeamsReached && (
          <Alert variant="destructive">
            <AlertTriangle className="h-4 w-4" />
            <AlertDescription>
              Você atingiu o limite de equipes do seu plano. 
              Faça upgrade para criar mais equipes.
            </AlertDescription>
          </Alert>
        )}
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                ...
                <DialogFooter className="pt-4">
                  ...
                  <Button type="submit" disabled={loading || (!isEditing && maxTeamsReached)}>
                    {loading ? (
