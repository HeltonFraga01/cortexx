/**
 * AgentListUser Component
 * 
 * Displays list of agents for user dashboard.
 * Reuses logic from admin AgentList.
 * 
 * Requirements: 2.1
 */
...
import type { Agent, AgentRole, AgentStatus, AvailabilityStatus, Team, Inbox, CustomRole, DatabaseConnection, DatabaseAccessConfig, Permission, AgentUpdates, DefaultRole } from '@/types/multi-user'
import { listAgents, deactivateAgent, activateAgent, updateAgentRole, getAgentDetails, updateAgentTeams, updateAgentInboxes, updateAgentDatabaseAccess, updateAgentPermissions } from '@/services/account-agents'
import { listTeams } from '@/services/account-teams'
import { listInboxes } from '@/services/account-inboxes'
import { listRoles } from '@/services/account-roles'
import { databaseConnectionsService } from '@/services/database-connections'
import { AgentInlineEditor } from './AgentInlineEditor'
import { AgentSummaryBadges } from './AgentSummaryBadges'
...
export function AgentListUser({ 
  ...
  const fetchAgents = useCallback(async () => {
    try {
      ...
      const [agentsData, teamsData, inboxesData, connectionsData, rolesData] = await Promise.all([
        listAgents(),
        listTeams().catch(() => []),
        listInboxes().catch(() => []),
        databaseConnectionsService.getUserConnections(userToken).catch(() => []),
        listRoles().catch(() => ({ defaultRoles: [], customRoles: [], availablePermissions: [] }))
      ])
      setAgents(agentsData)
      setTeams(teamsData)
      setInboxes(inboxesData)
      setDatabaseConnections(connectionsData.map((conn: any) => ({
        id: String(conn.id),
        name: conn.name,
        tableName: conn.table_name || conn.tableName
      })))
      setDefaultRoles(rolesData.defaultRoles || [])
  ...
  const fetchAgentDetails = useCallback(async (agentId: string) => {
    try {
      ...
      setAgentDetails(prev => ({
        ...
        [agentId]: {
          teams: details.teams,
          inboxes: details.inboxes,
          databaseAccess: details.databaseAccess,
  ...
  }, [expandedAgentId, agentDetails, fetchAgentDetails])
  
  const handleSaveAgentUpdates = useCallback(async (agentId: string, updates: AgentUpdates) => {
    try {
      if (updates.teamIds) {
        await updateAgentTeams(agentId, updates.teamIds)
      }
      if (updates.inboxIds) {
        await updateAgentInboxes(agentId, updates.inboxIds)
      }
      if (updates.databaseAccess) {
        await updateAgentDatabaseAccess(agentId, updates.databaseAccess)
      }
      if (updates.role || updates.customRoleId !== undefined) {
        await updateAgentPermissions(agentId, updates.role, undefined, updates.customRoleId)
      }
    ...
    <div className="space-y-6 w-full max-w-none mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {onInviteAgent && (
            <Button onClick={onInviteAgent} variant="outline" disabled={maxAgentsReached}>
              <MessageCircle className="h-4 w-4 mr-2 text-green-500" />
              Convidar
            </Button>
          )}
          {onCreateAgent && (
            <Button onClick={onCreateAgent} disabled={maxAgentsReached}>
                ...
                <Table className="w-full min-w-[800px]">
                  ...
                  <TableBody>
                    {paginatedAgents.map((agent) => {
                      ...
                      <TableRow key={agent.id} className="group hover:bg-muted/50">
                              ...
                              {details && (
                                <div className="mt-1">
                                  <AgentSummaryBadges
                                    ...
                                    inboxes={details.inboxes}
                                    databaseAccess={details.databaseAccess}
                                    onTeamClick={() => handleExpandAgent(agent.id)}
                                    onInboxClick={() => handleExpandAgent(agent.id)}
                                    onDatabaseClick={() => handleExpandAgent(agent.id)}
                                  />
                                </div>
                              )}
                            </div>
                        ...
                        <TableCell className="text-right px-3 sm:px-6">
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              variant={expandedAgentId === agent.id ? "default" : "outline"}
                              size="sm"
                      ...
                      {expandedAgentId === agent.id && details && (
                        <TableRow key={`${agent.id}-editor`}>
                          <TableCell colSpan={6} className="p-0">
                            <AgentInlineEditor
                              ...
                              currentTeamIds={details.teams.map(t => t.id)}
                              currentInboxIds={details.inboxes.map(i => i.id)}
                              currentDatabaseAccess={details.databaseAccess}
                              currentPermissions={details.permissions}
                              onSave={(updates) => handleSaveAgentUpdates(agent.id, updates)}
                              onCancel={() => setExpandedAgentId(null)}
                              isExpanded={true}
                            />
                          </TableCell>
                        </TableRow>
