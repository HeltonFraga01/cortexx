 ...
 * 
 * Botão para importar contatos da agenda WUZAPI para agentes.
 * Adapta a funcionalidade do ContactImportButton para o contexto de agentes.
 */
...
import { agentContactImportService } from '@/services/agentContactImportService';
...
export function AgentContactImportButton({
  instance,
  agentToken,
  inboxId,
  onImportComplete,
  ...
  const handleImport = async (isRetry: boolean = false) => {
    ...
    try {
      ...
      console.log('📥 Iniciando importação de contatos (agente)', {
        instance,
        inboxId,
      ...
      // Chamar serviço de importação para agentes
      const result = await agentContactImportService.importFromWuzapi(instance, agentToken, inboxId);

      console.log('✅ Importação bem-sucedida (agente)', {
        total: result.total,
      ...
      const successMessage = result.inboxName 
        ? `${result.total} contatos importados para ${result.inboxName}`
        : `${result.total} contatos importados com sucesso`;
      ...
      console.error('❌ Erro na importação de contatos (agente):', {
        error: err,
        instance,
        inboxId,
        retryCount,
  ...
  const handleRetry = () => {
    console.log('🔄 Retry manual iniciado (agente)');
    setRetryCount(0);
    setError(null);
    handleImport(false);
  };
  ...
  return (
    <Button 
      ...
      disabled={disabled || loading || !instance || !agentToken}
      variant={variant}
      size={size}
